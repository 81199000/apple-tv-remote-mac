...
class TouchHandler {
    ...
    /// Cursor movement multiplier, adjustable from the menu bar settings.
    ...
    private func findAndStartDevice() {
        ...
        // A Magic Trackpad is also reported as a non-built-in multitouch device.
        // The upstream implementation selected the first external device, which can
        // accidentally attach to the user's desktop trackpad. Siri Remote touch
        // surfaces are physically much smaller, so select the smallest external
        // surface and reject full-size trackpads.
        let candidates: [(device: MTDevice, area: Int)] = deviceList.compactMap { dev in
            guard !MTDeviceIsBuiltIn(dev) else { return nil }
            var width: Int32 = 0
            var height: Int32 = 0
            guard MTDeviceGetSensorSurfaceDimensions(dev, &width, &height) == 0,
                  width > 0,
                  height > 0,
                  max(width, height) < 10_000 else {
                return nil
            }
            return (dev, Int(width) * Int(height))
        }

        if let remote = candidates.min(by: { $0.area < $1.area })?.device {
            debugLog("findAndStartDevice: starting Siri Remote surface", data: ["deviceCount": deviceCount], hypothesisId: "B")
            startDevice(remote)
            return
        }

        // Never fall back to an arbitrary external device: doing so can capture a
        // Magic Trackpad when the remote is asleep or disconnected.
        do {
            // #region agent log
            debugLog("findAndStartDevice: no Siri Remote surface", data: ["deviceCount": deviceCount], hypothesisId: "B")
            // #endregion
    ...
    private func moveCursor(deltaX: CGFloat, deltaY: CGFloat) -> (clampedX: Bool, clampedY: Bool) {
        // Apply a continuous trackpad-like acceleration curve. Small per-frame
        // movements stay precise; faster/larger movements receive more travel.
        let magnitude = hypot(deltaX, deltaY)
        let normalizedSpeed = min(magnitude / 0.015, 1.0)
        let acceleration = 0.55 + 1.45 * pow(normalizedSpeed, 0.55)
        let scaledX = deltaX * cursorScale * acceleration
        let scaledY = -deltaY * cursorScale * acceleration
