...
class CursorController {
    ...
    /// Whether mapped key/click actions should play the macOS alert sound.
    /// Defaults to enabled to preserve the existing behavior.
    static var feedbackEnabled: Bool {
        get {
            UserDefaults.standard.object(forKey: "keyPressFeedbackEnabled") as? Bool ?? true
        }
        set {
            UserDefaults.standard.set(newValue, forKey: "keyPressFeedbackEnabled")
        }
    }
    ...
    func moveCursor(deltaX: CGFloat, deltaY: CGFloat) -> (clampedX: Bool, clampedY: Bool) {
        let scaledDeltaX = deltaX * sensitivity
        let scaledDeltaY = deltaY * sensitivity
